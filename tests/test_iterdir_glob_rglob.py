...
def test_iterdir(path_type: PathType, temp_dir_with_files, temp_local_dir, clean_remote_dir):
    ...
    remote_files = AnyPath(remote_dir).iterdir()
    assert sorted([fn.name for fn in remote_files]) == sorted([fn.name for fn in local_dir_files])
